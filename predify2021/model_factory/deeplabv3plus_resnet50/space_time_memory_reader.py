...
class SpaceTimeMemoryReader(nn.Module):
    ...
    def push(self, observation, memory):
        return (*memory, observation)[-self.max_memory:]
    ...
    def read(self, observation, memory):
        ...
        values = torch.stack(memory, dim=2)
        keys = torch.stack([self.key_projection(item) for item in memory], dim=2)
...
def detach_memory(memory):
    return tuple(value.detach() for value in memory)
