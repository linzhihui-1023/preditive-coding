...
3. Frozen C-V4 temporal hidden state + Dynamics Error（动力学误差）provide
   pixel-wise acceptance evidence only.
4. Semantic Writeback first produces a bounded proposal Delta-c4-sem. Temporal
   Reliability is applied AFTER Writeback and AFTER bounding, so g=0 strictly
   suppresses the deployed correction and g=1 passes the proposal unchanged.
5. Proposal and deployed residuals are both bounded by 0.10 x per-channel
   current-c4 RMS. Final segmentation is Decoder(c4 + Delta-c4-final).
"""
...
class PostWritebackReliabilityFeatureCorrector(nn.Module):
    """Prediction Error -> bounded c4 proposal; temporal state -> pixel acceptance."""
    ...
    def __init__(
        ...
        # Semantic content remains identical in role to C-V13: only e1..eK.
        ...
        # C-V14 changes C-V13's 128D pre-writeback modulation to one pixel-wise
        # post-writeback reliability logit. sigmoid(0)=0.5 at initialization.
    ...
    def forward(
        ...
        temporal_reliability_logit = self.temporal_reliability_head(temporal_latent)
        temporal_reliability = torch.sigmoid(temporal_reliability_logit)
        ...
        # Semantic proposal is produced independently of temporal reliability.
        raw_delta_c4_sem = self.writeback(current_c4, semantic_latent_c4)
        c4_channel_rms = (
            current_c4.square().mean(dim=(-2, -1), keepdim=True).sqrt().clamp_min(1e-6)
        )
        bounded_delta_c4_sem = (
            self.residual_scale
            * c4_channel_rms
            * torch.tanh(raw_delta_c4_sem)
        )
        proposal_c4 = current_c4 + bounded_delta_c4_sem

        # Reliability owns the deployed correction AFTER writeback and bounding.
        ...
        delta_c4 = reliability_c4 * bounded_delta_c4_sem
        ...
        return {
            ...
            "delta_c4": delta_c4,
            "bounded_delta_c4_sem": bounded_delta_c4_sem,
            "raw_delta_c4_sem": raw_delta_c4_sem,
            "c4_channel_rms": c4_channel_rms,
            "semantic_latent": semantic_latent,
            "temporal_latent": temporal_latent,
            "temporal_reliability_logit": temporal_reliability_logit,
