...
def validate_checkpoint(checkpoint, expected_revision):
    ...
    required_config = {
        ...
        "future_feature_temporal_fusion_architecture": ALIGNED_FUSION_ARCHITECTURE,
    ...
    allowed = ("future_feature_predictor.", "temporal_fusion_module.")
    if not names or not all(name.startswith(allowed) for name in names):
        raise ValueError("Aligned fusion has an unexpected trainable parameter family.")
    for prefix in allowed:
        if not any(name.startswith(prefix) for name in names):
            raise ValueError(f"Aligned fusion does not optimize {prefix}")
    return {
...
def evaluate_split(model, dataset, split, drive, fixed_dt_s):
    ...
    with torch.inference_mode():
        for sample_index, (current, futures, current_names, future_names) in enumerate(
            tqdm(loader, desc=f"aligned_temporal_fusion:{split}")
        ):
            ...
            alignment_applied = bool(outputs["alignment_applied"])
            raw_previous = outputs["raw_fusion_previous_top"]
            aligned_previous = outputs["aligned_previous_top"]
            ...
            rows.append(
                {
                    ...
                    "fusion_residual_rms": math.sqrt(
                        float(outputs["fusion_residual_top"].float().square().mean().item())
                    ),
...
def summarize_rows(rows):
    ...
    return {
        ...
        "temporal_fusion_applied_count": sum(
            bool(row["temporal_fusion_applied"]) for row in rows
        ),
...
def main():
    output_dir = Path(os.environ["PREDIFY_ALIGNED_FUSION_EVAL_OUTPUT_DIR"])
    ...
    checkpoint_path = Path(os.environ["PREDIFY_ALIGNED_FUSION_CHECKPOINT"])
    ...
    result = {
        "experiment": "kitti_stage5_aligned_two_frame_temporal_fusion",
        "git_revision": expected_revision,
        "device": str(DEVICE),
        "camera": camera,
        "fixed_dt_s": fixed_dt_s,
