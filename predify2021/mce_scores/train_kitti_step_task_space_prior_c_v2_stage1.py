...
from predify2021.mce_scores.train_kitti_step_task_space_prior_c_only import (
    FULL9, IGNORE_LABEL, NUM_CLASSES, _add_counts, _host_frame, _new_counts,
    _pair_mtc, _rates, _upsample_prior,
)
...
def _zero_step_check(model, transport, sample):
    ...
    with torch.no_grad():
        ...
        row = {
            ...
            "zero_flow_argmax_equals_persistence": bool(torch.equal(warped.argmax(1), host_low.argmax(1))),
...
def _train_sequence(model, transport, raft, samples, optimizer, args):
    if len(samples) < 2:
        return {"frames": 0, "windows": 0, "semantic_loss": 0.0, "flow_loss": 0.0, "total_loss": 0.0}
    ...
    pending_flow, hidden = transport.predict_next(previous_low.detach(), torch.zeros_like(previous_low), None)
    sem_losses, flow_losses = [], []
    sums = {"frames": 0, "windows": 0, "semantic_loss": 0.0, "flow_loss": 0.0, "total_loss": 0.0}
    ...
    for offset in range(steps):
        ...
        semantic_loss = F.cross_entropy(
            _upsample_prior(warped_low, output_size), target.unsqueeze(0), ignore_index=IGNORE_LABEL
        )
        with torch.no_grad():
            teacher_full = raft.current_to_previous(current_image, previous_image)
            teacher_low = downsample_backward_flow(teacher_full, tuple(current_low.shape[-2:]))
        ...
        next_flow, next_hidden = transport.predict_next(current_low.detach(), probability_error, previous_hidden)
        ...
        if boundary:
            ...
            pending_flow, hidden = transport.predict_next(
                current_low.detach(), probability_error.detach(),
...
def _train_epoch(model, transport, raft, groups, optimizer, args):
    transport.train()
    total = {"frames": 0, "windows": 0, "semantic_loss": 0.0, "flow_loss": 0.0, "total_loss": 0.0}
...
def _evaluate(model, transport, groups, raft):
    names = ("host", "semantic_persistence", "learned_transport", "repair_only_label_oracle")
    ...
    counts = _new_counts()
    flow_diag = {"pairs": 0, "values": 0, "pred_abs": 0.0, "teacher_abs": 0.0, "l1": 0.0, "pred_valid": 0.0}
    per_sequence = {}
    transport.eval()
    ...
    for sequence in FULL9:
        ...
        for index, sample in enumerate(groups[sequence]):
            ...
            if index == 0:
                pending_flow, hidden = transport.predict_next(host_low, torch.zeros_like(host_low), None)
                persist_pred = learned_pred = oracle_pred = host_pred
            else:
                ...
                teacher_low = downsample_backward_flow(teacher_full, tuple(host_low.shape[-2:]))
                _, teacher_valid = low_flow_grid(teacher_low)
                mask = teacher_valid.unsqueeze(1).expand_as(pending_flow)
                ...
                if nv:
                    ...
                    flow_diag["l1"] += float((pending_flow[mask] - teacher_low[mask]).abs().sum().item())
                flow_diag["pairs"] += 1
            ...
            if previous_image is not None:
                for n, pred in predictions.items():
                    ...
                    if math.isfinite(score):
                        mtc_sum[n] += score; mtc_n[n] += 1
                        seq_mtc_sum[n] += score; seq_mtc_n[n] += 1
    ...
    delta_host = {
        n: {k: metrics[n][k] - metrics["host"][k] for k in ("mIoU", "mTC", "mVC8", "mVC16")}
        for n in names if n != "host"
    }
    delta_persist = {
        k: metrics["learned_transport"][k] - metrics["semantic_persistence"][k]
        for k in ("mIoU", "mTC", "mVC8", "mVC16")
    }
    values, pairs = max(flow_diag["values"], 1), max(flow_diag["pairs"], 1)
    return {
        "metrics": metrics,
        ...
        "flow_diagnostics": {
            ...
            "predicted_abs_mean_low_pixels": flow_diag["pred_abs"] / values,
            "teacher_abs_mean_low_pixels": flow_diag["teacher_abs"] / values,
            "l1_mean_low_pixels": flow_diag["l1"] / values,
...
def _checks(result):
    ...
    return {
        ...
        "strong_reference_mIoU_ge_56p3pct": bool(learned["mIoU"] >= 0.563),
...
def main(argv=None):
    ...
    if args.epochs <= 0 or args.tbptt_steps <= 0 or args.max_displacement_low <= 0: raise ValueError("Invalid training arguments")
    random.seed(args.seed); torch.manual_seed(args.seed); torch.cuda.manual_seed_all(args.seed)

    model = load_fast_b_model(args.fast_b_checkpoint).cuda().eval(); model.requires_grad_(False)
    transport = CausalMotionTransportPredictor(
        num_classes=NUM_CLASSES, hidden_channels=HIDDEN_CHANNELS,
    ...
    train_groups = sequence_groups(train); all_val = sequence_groups(val)
    missing = [s for s in FULL9 if s not in all_val]
    if missing: raise RuntimeError(f"Missing Full9 validation sequences: {missing}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        row = {"epoch": epoch, "train": train_stats, "metrics": metrics, "stage1_checks": checks}
        history.append(row)
        payload = {
        ...
        if best is None or score > best["score"]:
            best = {"epoch": epoch, "score": score, "metrics": metrics, "stage1_checks": checks}
            torch.save(payload, output / "best.pt")
        (result_output / f"epoch_{epoch:03d}.json").write_text(json.dumps(row, indent=2, sort_keys=True) + "\n")
        print(json.dumps(row, sort_keys=True), flush=True)

    summary = {
        "experiment": "C-V2 Stage 1 Learned Motion Transport",
        "training": {
            "epochs": args.epochs, "tbptt_steps": args.tbptt_steps, "lr": args.lr,
            "weight_decay": args.weight_decay, "lambda_semantic": args.lambda_semantic,
            "lambda_flow": args.lambda_flow, "max_displacement_low": args.max_displacement_low,
        ...
        "raft_role": "training teacher and metric support only; absent from saved inference model",
        "zero_step": zero_step,
        "history": history,
        "best": best,
        "references": {"semantic_persistence": PERSISTENCE_REFERENCE, "raft_warped_persistence": RAFT_REFERENCE},
        "decision": "Proceed to C-V2 Stage 2 only if best.stage1_checks.stage1_minimum_go is true. 56.3% mIoU is a strong reference, not the minimum gate.",
    }
    ...
    print(json.dumps({
        ...
        "stage1_minimum_go": best["stage1_checks"]["stage1_minimum_go"] if best else False,
        "result": str(result_output / "summary.json"), "checkpoint": str(output / "best.pt"),
