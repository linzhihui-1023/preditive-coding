"""C-V14: post-writeback reliability acceptance for semantic feature correction.

中文：C-V14 = C-V4 时序参考 + K=4 预测误差语义修正 + 回写后可靠性接受控制。

Fixed experiment / 固定实验
---------------------------
- K=4 raw frozen C-V3 History Bank, motion aligned once.
- Frozen C-V4 supplies temporal hidden state and protection teacher.
- Semantic branch sees only e1..e4 and generates a bounded 0.10 x RMS(c4)
  feature proposal before any reliability control.
- Single-channel reliability g_t is applied AFTER bounded writeback, so g_t=0
  guarantees exact zero final correction.
- Training = all-pixel CE + C-V4-correct-region protection KL + training-only
  beneficial/harmful acceptance BCE, all weight 1.
- Acceptance BCE updates reliability_head only through detached temporal context.
...
EXPERIMENT = "c_v14_post_writeback_reliability_acceptance"
OUTPUT_DEFAULT = "/home/lin/predify/experiments/kitti_step_task_space_prior_c_v14_post_writeback_reliability_acceptance"
RESULT_DEFAULT = "results/kitti_step_task_space_prior_c_v14_post_writeback_reliability_acceptance"
EPOCHS = 3
...
def _architecture_metadata(corrector, c_v4_epoch):
    return {
        ...
        "semantic_proposal": "Host-conditioned writeback then 0.10 x per-channel RMS(c4) x tanh(raw proposal)",
        "temporal_context": "frozen C-V4 hidden + Dynamics Error + T/Q/validity/error agreement",
        "reliability": "single-channel pixel-wise sigmoid in [0,1]",
        "reliability_position": "after bounded semantic writeback",
        "feature_target": "DeepLabV3+ ResNet50 current c4, 2048 channels",
        "final_update": "Delta-c4-final = reliability * bounded semantic Delta-c4",
...
def main(argv=None):
    ...
    corrector = PostWritebackReliabilityFeatureCorrector(
    ...
    summary = {
        ...
        "architecture_decision": (
            "keep C-V13 semantic proposal and bound; move single-channel reliability "
            "after writeback and train it with beneficial/harmful acceptance evidence"
        ),
        "best": best,
        "history": history,
        "selection_rule": {
