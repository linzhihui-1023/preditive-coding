...
from predify2021.model_factory.deeplabv3plus_resnet50 import HostFeature, RoleSeparatedRecurrentPredictor, UnifiedFeatures, build_deeplabv3plus_resnet50_host
...
def observation_centered_posterior(observation, error, dynamic_error, corrections):
    gain_z1 = corrections[0](dynamic_error.z1)
    gain_z4 = corrections[1](dynamic_error.z4)
    return UnifiedFeatures(
        observation.z1 - gain_z1 * error.z1,
        observation.z2,
        observation.z3,
        observation.z4 - gain_z4 * error.z4,
    ), (gain_z1, gain_z4)


def residual_writeback_host_feature(model, raw_features, delta, output_size):
    zero = zero_state(delta)
    applied = model.decode_conditioned_adapter_deltas(raw_features, delta)
    baseline = model.decode_conditioned_adapter_deltas(raw_features, zero)
    return HostFeature(
        raw_features.c4 + applied.c4 - baseline.c4,
        raw_features.c1 + applied.c1 - baseline.c1,
        output_size,
    )
