...
def sequence_condition(seed, epoch, sequence_index):
    ...
    return rng.choice(TRAIN_CONDITIONS)


def apply_training_blur(image, condition):
    import torchvision.transforms.functional as TF
    return TF.gaussian_blur(
        image,
        [BLUR_KERNEL_SIZE, BLUR_KERNEL_SIZE],
        [BLUR_SIGMA[condition], BLUR_SIGMA[condition]],
    )


def encode_pair(model, sample, condition):
    clean = load_image(sample)
    noisy = apply_training_blur(clean, condition)
...
def calibrate_lambda(model, predictor, groups, semantic_parameters, chunks, tbptt_steps, seed, max_steps=0):
    ...
    for sequence_index, samples in enumerate(groups.values()):
        if len(samples) < 2:
            continue
        condition = sequence_condition(seed, 0, sequence_index)
        hidden = predictor.initial_state()
        pending = None
        observation, _, _, _ = encode_pair(model, samples[0], condition)
        kd_chunk, ref_chunk = [], []
        ...
        for t in range(steps):
            ...
            next_observation, clean_target, clean_logits, size = encode_pair(
                model, samples[target_index], condition
            )
...
def run_sequence(
    ...
    smoke_probe=None, condition_override=None,
):
    if len(samples) < 2:
        return 0, 0.0, 0.0, 0.0, check_dynamics_grad
    condition = condition_override or sequence_condition(seed, condition_epoch, sequence_index)
    hidden = predictor.initial_state()
    pending = None
    observation, _, _, _ = encode_pair(model, samples[0], condition)
    steps = min(len(samples) - 1, max_steps) if max_steps else len(samples) - 1
...
def run_epoch(
    ...
    for sequence_index, samples in enumerate(groups.values()):
        n, kd, ref, total, check_dynamics_grad = run_sequence(
            ...
            check_dynamics_grad, smoke_probe, condition_override,
        )
...
def is_better(val, best):
    if best is None or val["mean_decoded_mIoU"] > best["val"]["mean_decoded_mIoU"]:
        return True
    return (
        val["mean_decoded_mIoU"] == best["val"]["mean_decoded_mIoU"]
        and val["mean_combined_loss"] < best["val"]["mean_combined_loss"]
    )
...
def main():
    ...
    for epoch in range(1, epochs + 1):
        ...
        val_metrics = {
            "conditions": val_by_condition,
            "mean_decoded_mIoU": sum(
                metrics["decoded_mIoU"] for metrics in val_by_condition.values()
            ) / len(val_by_condition),
            "mean_combined_loss": sum(
                metrics["combined_loss"] for metrics in val_by_condition.values()
            ) / len(val_by_condition),
        }
    ...
    summary = {
        ...
        "config": {
            ...
            "augmentation": "Persistent Gaussian blur on Semantic Branch training inputs",
            "training_conditions": {"Blur-Mid": "sigma=2.25", "Blur-Max": "sigma=3.0"},
            "corruption_scope": "one of Blur-Mid/Blur-Max fixed within each sequence; assignment may change by epoch",
            "validation_conditions": ["Blur-Mid", "Blur-Max"],
            "validation_selection": "highest mean decoded mIoU across Blur-Mid/Blur-Max; lower mean combined loss breaks exact ties",
        },
