"""C-V16-B: scoped joint training of the C-V15 proposal content path + gate.

Same stack as C-V16-A, but in addition to the freshly reset gate this run also
unfreezes the proposal *content* path (``semantic_encoder`` + ``writeback``) so
proposal and gate can co-adapt. Everything else stays frozen: Host, C-V3,
motion observer/residual, Euler dynamics, C-V4 controller, and ``temporal_pre``
(the reliability-prior evidence encoder). Objective remains L_seg only.

Hypothesis: C-V16-A showed a frozen proposal + retrained gate cannot beat C-V4
on clean KITTI-STEP. This tests whether the staged freezing of proposal and
...
EXPERIMENT = "c_v16_b_joint_proposal_gate"
...
OUTPUT_DEFAULT = (
    "/home/lin/predify/experiments/"
    "kitti_step_task_space_prior_c_v16_b_joint_proposal_gate"
)
RESULT_DEFAULT = "/home/lin/predify/results/kitti_step_task_space_prior_c_v16_b_joint_proposal_gate"
EPOCHS = 6
...
def _architecture_metadata(corrector, cv15_path, cv15_payload, c_v4_epoch):
    return {
        "name": "C-V16-B Scoped Joint Proposal-Content + Gate",
        "causal_path": "C-V15-init Proposal content (co-adapted) -> Fresh Reliability Gate -> Corrected C4 -> Decoder",
        ...
        "proposal_generator_frozen": False,
        "trainable_modules": [
            "semantic_encoder", "writeback", "reliability_head",
            "proposal_encoder", "host_encoder", "acceptance_fusion",
            "acceptance_residual_head",
        ],
        "frozen_temporal_prior_encoder": "temporal_pre",
        ...
        "final_composition": "Decoder(c4 + g_t * bounded co-adapted semantic Delta-c4)",
        "reporting_boundary": (
            "Tests whether staged freezing of proposal content and gate is the "
            "limiting factor vs C-V16-A. Upstream (Host, C-V3, motion, dynamics, "
            "C-V4, temporal_pre) stays frozen; this is not from-scratch joint training."
        ),
    }
...
def main(argv=None):
    ...
    # C-V16-B: additionally co-adapt the proposal *content* path. temporal_pre
    # (the reliability-prior evidence encoder) stays frozen; every upstream
    # module stays frozen.
    corrector.semantic_encoder.requires_grad_(True)
    corrector.writeback.requires_grad_(True)
    corrector._proposal_frozen = False
    initialization = corrector.contract_state()
    if not initialization["gate_initialized_after_checkpoint_loading"]:
        raise RuntimeError("C-V16-B Gate was not reset after checkpoint loading")
    if any(p.requires_grad for p in corrector.temporal_pre.parameters()):
        raise RuntimeError("C-V16-B temporal_pre must stay frozen")
    _trainable = {
        name.split(".")[0]
    ...
    if _trainable != _expected:
        raise RuntimeError(
            f"C-V16-B unexpected trainable set: {sorted(_trainable)}"
        )
    initialization["c_v16_b_trainable_modules"] = sorted(_trainable)
    initialization["c_v16_b_trainable_parameter_count"] = sum(
        p.numel() for p in corrector.parameters() if p.requires_grad
    )
    print(
        "[C-V16-B] trainable:", sorted(_trainable),
        "params:", initialization["c_v16_b_trainable_parameter_count"],
        flush=True,
    )
