...
from predify2021.mce_scores.semantic_temporal_error_step import detach_error_state, semantic_temporal_error_step, zero_dynamic_transition_state, zero_error_state
...
from predify2021.model_factory.deeplabv3plus_resnet50.semantic_temporal_error_correction import build_dpc_semantic_temporal_corrections, build_semantic_temporal_corrections
...
BASELINE_CHECKPOINT = "/home/lin/experiments/kitti_step_semantic_temporal_error_correction_10epoch_ffb1a74/best_semantic_temporal_error_correction.pt"
def load_new(path):
...
def run_blur_benchmark(model, predictor, baseline, corrections, groups, sigmas):
    method_names = ("corrupted_host", "baseline_ours", "dpc_full")
    results = {
        "clean": {
            ...
            for samples in groups.values():
                ...
                for frame_index, sample in enumerate(samples):
                    ...
                    if baseline_hidden is None:
                        baseline_hidden = zero_error_state(observation)
                        dpc_state = zero_dynamic_transition_state(observation)
                    if frame_index == 0:
                    ...
                    if frame_index == 1:
                        ...
                        continue
                    baseline_posterior, baseline_hidden, _ = semantic_temporal_error_step(
                        baseline,
                        observation,
                        pending_dynamics,
                        pending_semantic,
                    ...
                    dpc_posterior, dpc_state, _ = semantic_temporal_error_step(
                        corrections,
                        observation,
                        pending_dynamics,
                        pending_semantic,
                    ...
                    hosts = {
                        "corrupted_host": raw_host,
                        "baseline_ours": host_from_delta(
                            model,
                            raw,
                            UnifiedFeatures(
                                baseline_posterior.z1 - observation.z1,
                                zero.z2,
                                zero.z3,
                                baseline_posterior.z4 - observation.z4,
                            ),
                            output_size,
                        ),
                        "dpc_full": host_from_delta(
                            model,
                            raw,
                            UnifiedFeatures(
                                dpc_posterior.z1 - observation.z1,
                                zero.z2,
                                zero.z3,
                                dpc_posterior.z4 - observation.z4,
                            ),
                            output_size,
                        ),
                    }
                    ...
                    predictions = {
                        name: model.decode_from_host_feature(host).argmax(1).squeeze(0).cpu()
                    ...
                    pending_dynamics, pending_semantic, *predictor_hidden = next_role_prediction(predictor, observation, error, predictor_hidden)
                    baseline_hidden = detach_error_state(baseline_hidden)
                    dpc_state = detach_error_state(dpc_state)
...
def corruption_degradation(benchmark, clean_key, reference_name="corrupted_host"):
    ...
    output = {}
    for name in benchmark[clean_key]:
...
def dpc_decision(metrics, blur_benchmark, blur_robustness):
    baseline = metrics["baseline_ours"]
    full = metrics["dpc_full"]
    zero = metrics["dpc_zero_dynamic"]
    full_minus_zero_miou = full["miou"] - zero["miou"]
    full_minus_zero_mvc16 = full["mvc16"] - zero["mvc16"]
    dynamic_contribution = (
        full_minus_zero_miou >= 0.002 or full_minus_zero_mvc16 >= 0.002
    )
    primary = (
        full["mvc16"] - baseline["mvc16"] >= 0.005
        and full["miou"] >= baseline["miou"] - 0.002
        and blur_robustness["dpc_full"]["mean_blur_miou"]
        >= blur_robustness["baseline_ours"]["mean_blur_miou"] - 0.002
        and full["mvc8"] >= baseline["mvc8"] - 0.002
        and blur_benchmark["S1"]["dpc_full"]["miou"]
        >= blur_benchmark["S1"]["baseline_ours"]["miou"] - 0.005
        and dynamic_contribution
    )
    strong = primary and (
        full["mvc16"] - baseline["mvc16"] >= 0.010
        or (
            full["miou"] - baseline["miou"] >= 0.005
            and full["mvc16"] > baseline["mvc16"]
        )
    )
    no_go = (
        full["mvc16"] < baseline["mvc16"]
        or full["miou"] < baseline["miou"] - 0.005
        or blur_robustness["dpc_full"]["mean_blur_miou"]
        < blur_robustness["baseline_ours"]["mean_blur_miou"] - 0.005
        or not dynamic_contribution
    )
    if strong:
        decision = "DPC_TRANSITION: STRONG GO"
    elif primary:
        decision = "DPC_TRANSITION: GO"
    elif no_go:
        decision = "DPC_TRANSITION: NO-GO"
    else:
        decision = "DPC_TRANSITION: INCONCLUSIVE"
    return {
        "decision": decision,
        "primary_go": primary,
        "strong_go": strong,
        "dynamic_error_independent_contribution": dynamic_contribution,
        "dpc_full_minus_baseline_miou": full["miou"] - baseline["miou"],
        "dpc_full_minus_baseline_mvc8": full["mvc8"] - baseline["mvc8"],
        "dpc_full_minus_baseline_mvc16": full["mvc16"] - baseline["mvc16"],
        "dpc_full_minus_zero_dynamic_miou": full_minus_zero_miou,
        "dpc_full_minus_zero_dynamic_mvc16": full_minus_zero_mvc16,
        "dpc_full_minus_baseline_mean_blur_miou": (
            blur_robustness["dpc_full"]["mean_blur_miou"]
            - blur_robustness["baseline_ours"]["mean_blur_miou"]
        ),
    }
...
def main():
    ...
    output = Path(os.environ.get("PREDIFY_DPC_TRANSITION_EVAL_OUTPUT_DIR", "results/kitti_step_dpc_transition_correction"))
    checkpoint = Path(os.environ["PREDIFY_DPC_TRANSITION_CHECKPOINT"])
    paths = make_paths()
    ...
    baseline_path = Path(os.environ.get("PREDIFY_SEMANTIC_TEMPORAL_ERROR_BASELINE_CHECKPOINT", BASELINE_CHECKPOINT))
        ...
        for samples in groups.values():
            ...
            for frame_index, sample in enumerate(samples):
                ...
                if baseline_hidden is None:
                    baseline_hidden = zero_error_state(observation)
                    dpc_state = zero_dynamic_transition_state(observation)
                    zero_dynamic_state = zero_dynamic_transition_state(observation)
                if frame_index == 0:
                    pending_dynamics, pending_semantic, *predictor_hidden = next_role_prediction(predictor, observation, zero_state(observation), predictor_hidden)
                    ...
                    _, _, zero_values = semantic_temporal_error_step(
                        ...
                        observation,
                        observation,
                        zero_dynamic_transition_state(observation),
                    )
                    full_zero_error_max = max(
                ...
                if frame_index == 1:
                    ...
                    continue
                baseline_posterior, baseline_hidden, _ = semantic_temporal_error_step(
                    baseline, observation, pending_dynamics, pending_semantic, baseline_hidden
                )
                posterior, dpc_state, values = semantic_temporal_error_step(
                    corrections, observation, pending_dynamics, pending_semantic, dpc_state
                )
                zero_dynamic_posterior, zero_dynamic_state, _ = semantic_temporal_error_step(
                    corrections,
                    observation,
                    pending_dynamics,
                    pending_semantic,
                ...
                if frame_index < warmup_frame_count(len(samples)):
                    ...
                    baseline_hidden = detach_error_state(baseline_hidden)
                    dpc_state = detach_error_state(dpc_state)
                    zero_dynamic_state = detach_error_state(zero_dynamic_state)
                ...
                hosts = {
                    ...
                    "baseline_ours": host_from_delta(model, corrupted_raw, UnifiedFeatures(baseline_posterior.z1 - observation.z1, zero_delta.z2, zero_delta.z3, baseline_posterior.z4 - observation.z4), output_size),
                    "dpc_full": host_from_delta(model, corrupted_raw, UnifiedFeatures(posterior.z1 - observation.z1, zero_delta.z2, zero_delta.z3, posterior.z4 - observation.z4), output_size),
                    "dpc_zero_dynamic": host_from_delta(model, corrupted_raw, UnifiedFeatures(zero_dynamic_posterior.z1 - observation.z1, zero_delta.z2, zero_delta.z3, zero_dynamic_posterior.z4 - observation.z4), output_size),
                }
                ...
                logits = {
                    ...
                    "baseline_ours": model.decode_from_host_feature(hosts["baseline_ours"]),
                    "dpc_full": model.decode_from_host_feature(hosts["dpc_full"]),
                    "dpc_zero_dynamic": model.decode_from_host_feature(hosts["dpc_zero_dynamic"]),
                }
                mask = semantic_mask_from_panoptic_png(sample["mask_path"])
                for name in names:
                ...
                values_for_total = {
                    ...
                    "mean_abs_hidden_z1": dpc_state.hidden[0].abs().mean(), "mean_abs_hidden_z4": dpc_state.hidden[1].abs().mean(),
                    "mean_abs_dynamic_error_z1": dpc_state.dynamic_error[0].abs().mean(), "mean_abs_dynamic_error_z4": dpc_state.dynamic_error[1].abs().mean(),
                    ...
                    "mean_transition_weight_v1_z1": values["transition_weights_z1"][0], "mean_transition_weight_v2_z1": values["transition_weights_z1"][1], "mean_transition_weight_v3_z1": values["transition_weights_z1"][2],
                    "mean_transition_weight_v1_z4": values["transition_weights_z4"][0], "mean_transition_weight_v2_z4": values["transition_weights_z4"][1], "mean_transition_weight_v3_z4": values["transition_weights_z4"][2],
                    "mean_transition_entropy_z1": values["transition_entropy_z1"], "mean_transition_entropy_z4": values["transition_entropy_z4"],
                ...
                finite = finite and all(torch.isfinite(value).all().item() for value in (*values_for_total.values(), *logits.values(), *dpc_state.hidden, *dpc_state.dynamic_error))
    ...
    blur_benchmark = run_blur_benchmark(model, predictor, baseline, corrections, groups, IMAGENET_C_GAUSSIAN_BLUR_SIGMAS)
    ...
    decision = dpc_decision(metrics, blur_benchmark, blur_robustness)
    ...
    result = {
        "experiment": "kitti_step_dpc_transition_correction",
        ...
        "mIoU_new_minus_corrupted_host": metrics["dpc_full"]["miou"] - metrics["corrupted_host"]["miou"],
        "mVC8_new_minus_corrupted_host": metrics["dpc_full"]["mvc8"] - metrics["corrupted_host"]["mvc8"],
        "mVC16_new_minus_corrupted_host": metrics["dpc_full"]["mvc16"] - metrics["corrupted_host"]["mvc16"],
        "recovery_ratio": (metrics["dpc_full"]["miou"] - metrics["corrupted_host"]["miou"]) / (metrics["clean_host"]["miou"] - metrics["corrupted_host"]["miou"]),
        ...
        "parameter_efficiency": {"baseline_correction_trainable_params": baseline_params, "dpc_correction_trainable_params": dpc_params, "new_parameters": dpc_params - baseline_params, "additional_inference_params": dpc_params - baseline_params},
        "diagnostics": totals,
        "gates": {
