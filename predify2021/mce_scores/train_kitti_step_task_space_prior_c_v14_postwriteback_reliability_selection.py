"""C-V14: post-writeback pixel reliability for semantic feature correction.

中文：C-V14 = C-V13 有界语义特征修正 + 回写后像素级时序可靠性选择。
...
- Semantic Branch sees only e1..e4 and produces bounded Delta-c4-sem.
- Pixel-wise Reliability g_t in [0,1] is applied after Writeback + 10% bound.
- Training = all-pixel CE + C-V4-correct protection KL + explicit
  beneficial/harmful Reliability BCE, all fixed weight 1.
- Reliability target uses detached proposal; GT is training-only target logic.
...
EXPERIMENT = "c_v14_postwriteback_reliability_selection"
OUTPUT_DEFAULT = "/home/lin/predify/experiments/kitti_step_task_space_prior_c_v14_postwriteback_reliability_selection"
RESULT_DEFAULT = "results/kitti_step_task_space_prior_c_v14_postwriteback_reliability_selection"
EPOCHS = 3
...
def _architecture_metadata(corrector, c_v4_epoch):
    return {
        ...
        "semantic_proposal": "Host-conditioned Writeback -> tanh -> 0.10 x per-channel RMS(c4)",
        "temporal_context": "frozen C-V4 hidden + Dynamics Error + T/Q/validity/error agreement",
        "temporal_reliability": "1D pixel-wise sigmoid in [0,1], post-writeback",
        "reliability_application": "Delta-c4-final = g_t * bounded Delta-c4-sem",
...
def main(argv=None):
    ...
    summary = {
        "experiment": "C-V14 Post-Writeback Reliability Selection",
        "architecture_decision": (
            "keep C-V13 semantic proposal and bound; move temporal Reliability after bounded writeback "
            "and supervise it with detached beneficial/harmful proposal targets"
        ),
        "best": best,
        "history": history,
        "selection_rule": {
