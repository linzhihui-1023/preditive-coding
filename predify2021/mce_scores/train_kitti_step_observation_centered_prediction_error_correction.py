...
def identity_gate(model, sample):
    with torch.no_grad():
        ...
        raw = model.extract_backbone_features(noisy_image)
        output_size = tuple(noisy_image.shape[-2:])
        noisy_host = HostFeature(raw.c4, raw.c1, output_size)
        zero_delta = zero_state(model.encode_backbone_features(raw))
        identity_host = residual_writeback_host_feature(model, raw, zero_delta, output_size)
        noisy_logits = model.decode_from_host_feature(noisy_host)
        identity_logits = model.decode_from_host_feature(identity_host)
        max_difference = (identity_logits - noisy_logits).abs().max().item()
    return {"max_abs_logit_difference": max_difference, "passed": max_difference <= 1e-6}
...
def run_epoch(model, predictor, corrections, groups, optimizer=None):
    ...
    for samples in groups.values():
        ...
        for frame_index, sample in enumerate(samples):
            ...
            posterior, _ = observation_centered_posterior(
...
def main():
    if not torch.cuda.is_available():
        raise RuntimeError("Observation-centered correction training requires CUDA")
    random.seed(0)
    torch.manual_seed(0)
    torch.cuda.manual_seed_all(0)
    root = Path(os.environ.get("PREDIFY_KITTI_STEP_ROOT", "/home/lin/predify/kitti_step"))
    output = Path(os.environ.get("PREDIFY_OBSERVATION_CENTERED_CORRECTION_OUTPUT_DIR", "/home/lin/predify/experiments/kitti_step_observation_centered_prediction_error_correction"))
    ...
    gate = identity_gate(model, val.samples[2])
    if not gate["passed"]:
        raise RuntimeError("CORRECTION_BASELINE_IDENTITY: NO-GO")
    history = []
    best = None
    for epoch in range(1, EPOCHS + 1):
        ...
        record = {"epoch": epoch, "train_correction_state_mse": train_loss, "train_frame_count": train_frames, "val_correction_state_mse": val_loss, "val_frame_count": val_frames}
        history.append(record)
        print(json.dumps(record, sort_keys=True), flush=True)
        if best is None or val_loss < best["val_correction_state_mse"]:
            best = record
            output.mkdir(parents=True, exist_ok=True)
            torch.save({"correction_state_dict": corrections.state_dict(), "epoch": epoch, "val_correction_state_mse": val_loss, "identity_gate": gate}, output / "best_observation_centered_correction.pt")
    summary = {"experiment": "kitti_step_observation_centered_prediction_error_correction_training", "git_revision": os.environ.get("PREDIFY_GIT_REVISION"), "checkpoint": str(output / "best_observation_centered_correction.pt"), "static_host_checkpoint": static_checkpoint, "adapter_checkpoint": adapter_checkpoint, "predictor_checkpoint": predictor_checkpoint, "writeback_checkpoint": writeback_checkpoint, "trainable_parameter_count": sum(parameter.numel() for parameter in parameters), "identity_gate": gate, "config": {"epochs": EPOCHS, "optimizer": "AdamW", "learning_rate": LEARNING_RATE, "weight_decay": WEIGHT_DECAY, "seed": 0, "gaussian_noise_sigma": SIGMA, "alpha": 0.207, "beta": 0.793, "labels_used": False}, "dataset": {"train_sequence_count": len(train_groups), "val_sequence_count": len(val_groups), "train_frame_count": len(train.samples), "val_frame_count": len(val.samples)}, "history": history, "best": best}
    output.mkdir(parents=True, exist_ok=True)
