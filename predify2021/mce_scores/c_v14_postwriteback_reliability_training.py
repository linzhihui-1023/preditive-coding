"""Training/evaluation helpers for C-V14 post-writeback reliability selection.

中文：C-V14 回写后可靠性选择训练与评测辅助模块。

C-V14 keeps C-V13's K=4 Prediction-Error semantic proposal and 10% bounded
c4 residual. The only structural change is that temporal Reliability is a
pixel-wise [0,1] acceptance map applied after bounded semantic writeback.
Training = all-pixel CE + C-V4-correct protection KL + beneficial/harmful
acceptance BCE. Acceptance targets are training-only and proposal-detached.
"""
...
@torch.no_grad()
def decode_detached_proposal(model, observation, proposal_c4):
    """Decode full semantic proposal with no gradient into the proposal branch."""
...
@torch.no_grad()
def acceptance_targets(current_logits, proposal_logits, gt_gpu):
    """Training-only unambiguous proposal acceptance targets.

    positive: current wrong -> full proposal correct.
    negative: current correct -> full proposal wrong.
    all other pixels are ignored.
    """
    valid = gt_gpu.ne(IGNORE_LABEL)
    current_pred = current_logits.detach().argmax(1)[0]
    proposal_pred = proposal_logits.detach().argmax(1)[0]
    positive = valid & current_pred.ne(gt_gpu) & proposal_pred.eq(gt_gpu)
    negative = valid & current_pred.eq(gt_gpu) & proposal_pred.ne(gt_gpu)
    supervised = positive | negative
    target = positive.to(dtype=current_logits.dtype)
    return {
        "target": target,
...
def reliability_bce_loss(reliability_logit_low, target_row, output_size):
    ...
    supervised = target_row["supervised"]
    supervised_pixels = int(supervised.sum().item())
    if supervised_pixels == 0:
        return logit_full.sum() * 0.0, logit_full
    loss = F.binary_cross_entropy_with_logits(
        logit_full[supervised],
        target_row["target"][supervised],
        reduction="mean",
    )
    return loss, logit_full
...
def train_sequence(
    ...
    for frame_index in range(2, len(samples)):
        ...
        final_logits, correction_row = forward_feature_update(
            model, corrector, observation, error_row,
            e1["transportability_low"], memory_row["memory_reliability"],
        )
        # Proposal target is detached by construction: reliability BCE cannot
        # backpropagate into Semantic Branch / Writeback.
        proposal_logits = decode_detached_proposal(
            model, observation, correction_row["proposal_c4"]
        )
        ...
        acceptance = acceptance_targets(
            observation["host_logits"], proposal_logits, target[0]
        )
        reliability_bce, reliability_logit_full = reliability_bce_loss(
            correction_row["temporal_reliability_logit"],
            acceptance,
            observation["output_size"],
        )
        loss = (
        ...
        with torch.no_grad():
            ...
            reliability_full = torch.sigmoid(reliability_logit_full)
            ...
            totals["reliability_bce"] += float(reliability_bce.item())
            ...
            totals["acceptance_supervised_pixels"] += positive_count + negative_count
            if positive_count:
                totals["positive_reliability_sum"] += float(reliability_full[positive].sum().item())
                totals["positive_gate_accept"] += int((reliability_full[positive] >= 0.5).sum().item())
            if negative_count:
                totals["negative_reliability_sum"] += float(reliability_full[negative].sum().item())
                totals["negative_gate_reject"] += int((reliability_full[negative] < 0.5).sum().item())

            current = observation["c4"]
            current_rms = current.square().mean().sqrt().clamp_min(1e-8)
            proposal_delta = correction_row["bounded_delta_c4_sem"]
            ...
            totals["proposal_delta_logits_abs"] += float(
                (proposal_logits - observation["host_logits"]).abs().mean().item()
            )
            totals["feature_delta_logits_abs"] += float(
                (final_logits.detach() - observation["host_logits"]).abs().mean().item()
            )
            totals["temporal_reliability_mean"] += float(reliability_full.mean().item())
            totals["temporal_reliability_std"] += float(reliability_full.std().item())
    ...
    for key in (
        "final_ce", "protection_kl", "reliability_bce",
        "proposal_delta_c4_rms_ratio", "delta_c4_rms_ratio",
        "proposal_delta_logits_abs", "feature_delta_logits_abs",
        "temporal_reliability_mean", "temporal_reliability_std",
    ):
        totals[key] /= frames
    ...
    totals["positive_accept_recall_at_0_5"] = totals["positive_gate_accept"] / max(totals["acceptance_positive_pixels"], 1)
    totals["negative_reject_rate_at_0_5"] = totals["negative_gate_reject"] / max(totals["acceptance_negative_pixels"], 1)
    return totals
...
def train_epoch(
    ...
    for key in (
        ...
        "baseline_correct_damaged", "acceptance_positive_pixels",
        "acceptance_negative_pixels", "acceptance_supervised_pixels",
        "positive_gate_accept", "negative_gate_reject",
    ):
        result[key] = sum(row[key] for row in rows)
    result["positive_reliability_sum"] = sum(row["positive_reliability_sum"] for row in rows)
    result["negative_reliability_sum"] = sum(row["negative_reliability_sum"] for row in rows)
    ...
    result["positive_reliability_mean"] = result["positive_reliability_sum"] / max(result["acceptance_positive_pixels"], 1)
    result["negative_reliability_mean"] = result["negative_reliability_sum"] / max(result["acceptance_negative_pixels"], 1)
    result["positive_accept_recall_at_0_5"] = result["positive_gate_accept"] / max(result["acceptance_positive_pixels"], 1)
    result["negative_reject_rate_at_0_5"] = result["negative_gate_reject"] / max(result["acceptance_negative_pixels"], 1)
    result["protection_loss_weight"] = PROTECTION_LOSS_WEIGHT
    result["reliability_loss_weight"] = RELIABILITY_LOSS_WEIGHT
...
def evaluate(
    ...
    for sequence in FULL9:
        ...
        for frame_index in range(2, len(samples)):
            ...
            acceptance = acceptance_targets(
                observation["host_logits"], proposal_logits, gt_gpu
            )
            _, reliability_logit_full = reliability_bce_loss(
                correction_row["temporal_reliability_logit"],
                acceptance,
                observation["output_size"],
            )
            ...
            diag["acceptance_supervised_pixels"] += positive_count + negative_count
            if positive_count:
                diag["positive_reliability_sum"] += float(reliability_full[positive].sum().item())
                diag["positive_gate_accept"] += int((reliability_full[positive] >= 0.5).sum().item())
            if negative_count:
                diag["negative_reliability_sum"] += float(reliability_full[negative].sum().item())
                diag["negative_gate_reject"] += int((reliability_full[negative] < 0.5).sum().item())

            current_rms = observation["c4"].square().mean().sqrt().clamp_min(1e-8)
            diag["proposal_delta_c4_rms_ratio"] += float(
                correction_row["bounded_delta_c4_sem"].square().mean().sqrt().div(current_rms).item()
            )
            diag["delta_c4_rms_ratio"] += float(
                correction_row["delta_c4"].square().mean().sqrt().div(current_rms).item()
            )
            diag["proposal_delta_logits_abs"] += float(
                (proposal_logits - observation["host_logits"]).abs().mean().item()
            )
            diag["feature_delta_logits_abs"] += float(
                (final_logits - observation["host_logits"]).abs().mean().item()
            )
            diag["temporal_reliability_mean"] += float(reliability_full.mean().item())
            diag["temporal_reliability_std"] += float(reliability_full.std().item())
    ...
    diag["positive_accept_recall_at_0_5"] = diag["positive_gate_accept"] / max(diag["acceptance_positive_pixels"], 1)
    diag["negative_reject_rate_at_0_5"] = diag["negative_gate_reject"] / max(diag["acceptance_negative_pixels"], 1)
    diag.update({
        ...
        "semantic_proposal": "bounded Delta-c4-sem produced before Reliability",
        "temporal_reliability": "pixel-wise sigmoid applied after bounded writeback",
        "acceptance_target": "current-wrong/proposal-correct positive; current-correct/proposal-wrong negative; others ignored",
        "acceptance_target_gt_training_only": True,
