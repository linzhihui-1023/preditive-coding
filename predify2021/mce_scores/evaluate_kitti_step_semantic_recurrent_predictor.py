...
def main():
    ...
    adapter = torch.load(ADAPTER_CHECKPOINT_DEFAULT, map_location="cpu", weights_only=False); model.multi_layer_adapter.load_state_dict(adapter["adapter_state_dict"], strict=True); load_writeback_checkpoint(model, Path(WRITEBACK)); model.eval()
    ...
    with torch.inference_mode():
        for samples in groups.values():
            vc.reset_sequence(); previous_previous = previous = None; hidden4 = hidden1 = None; new_error = None; pending = None
            for index, sample in enumerate(samples):
                ...
                if previous is None:
                    previous = current; pending, hidden4, hidden1 = new.step(current, UnifiedFeatures(*(torch.zeros_like(v) for v in current.as_tuple())), hidden4, hidden1); continue
                if previous_previous is None:
                    previous_previous, previous = previous, current; old_pred, _ = predict_current(old, previous_previous, previous, current); new_error = UnifiedFeatures(*(a - b for a, b in zip(current.as_tuple(), pending.as_tuple()))); pending, hidden4, hidden1 = new.step(current, new_error, hidden4, hidden1); continue
                ...
                vc.append(mask, predictions); new_error = UnifiedFeatures(*(a - b for a, b in zip(current.as_tuple(), new_pred.as_tuple())))
                new_error = UnifiedFeatures(*(a - b for a, b in zip(current.as_tuple(), new_pred.as_tuple()))); pending, hidden4, hidden1 = new.step(current, new_error, hidden4, hidden1)
