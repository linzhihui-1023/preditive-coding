"""Training/evaluation helpers for C-V13 bounded reliability feature update.

中文：C-V13 有界可靠性特征更新训练与评测辅助模块。

C-V13 keeps the frozen C-V4 temporal core as temporal reference and teacher,
but final segmentation is decoded directly from corrected current c4.
Training uses all-pixel segmentation CE + C-V4-correct-region protection KL.
There is no Rescue CE, no temporal loss, and no RAFT training teacher.
"""
...
def decode_bounded_feature_update(
    model,
    corrector,
    observation,
    error_row,
...
def protection_kl_loss(final_logits, c_v4_logits, gt_gpu):
    ...
    protect_pixels = int(protect_mask.sum().item())
    if protect_pixels == 0:
...
def train_sequence(
    ...
    for frame_index in range(2, len(samples)):
        ...
        final_logits, feature_delta_logits, correction_row = decode_bounded_feature_update(
        ...
        if not torch.isfinite(loss):
            raise FloatingPointError("Non-finite C-V13 bounded feature loss")
        loss.backward()
        frames_in_window += 1
        ...
        with torch.no_grad():
            ...
            totals["delta_c4_rms_ratio"] += float(
                delta.square().mean().sqrt().div(
                    current.square().mean().sqrt().clamp_min(1e-8)
                ).item()
            )
            totals["raw_delta_c4_abs"] += float(correction_row["raw_delta_c4"].abs().mean().item())
            totals["feature_delta_logits_abs"] += float(feature_delta_logits.abs().mean().item())
            totals["temporal_reliability_mean"] += float(correction_row["temporal_reliability"].mean().item())
            totals["temporal_reliability_std"] += float(correction_row["temporal_reliability"].std().item())
    ...
    for key in (
        "final_ce", "protection_kl", "delta_c4_abs", "delta_c4_rms_ratio",
        "raw_delta_c4_abs", "feature_delta_logits_abs",
...
def train_epoch(
    ...
    for samples in groups.values():
        ...
        if row is not None:
            rows.append(row)
    if not rows:
        raise RuntimeError("No valid C-V13 training sequences")
...
def evaluate(
    ...
    names = ("host", "c_v3_base", "c_v4_frozen", "c_v13")
    ...
    for sequence in FULL9:
        ...
        for frame_index in range(2, len(samples)):
            ...
            diag["delta_c4_rms_ratio"] += float(
                delta.square().mean().sqrt().div(
                    observation["c4"].square().mean().sqrt().clamp_min(1e-8)
                ).item()
            )
            diag["raw_delta_c4_abs"] += float(correction_row["raw_delta_c4"].abs().mean().item())
            diag["feature_delta_logits_abs"] += float(feature_delta_logits.abs().mean().item())
            diag["temporal_reliability_mean"] += float(correction_row["temporal_reliability"].mean().item())
            diag["temporal_reliability_std"] += float(correction_row["temporal_reliability"].std().item())
    ...
    for key in (
        "delta_c4_abs", "delta_c4_rms_ratio", "raw_delta_c4_abs",
        "feature_delta_logits_abs", "temporal_reliability_mean", "temporal_reliability_std",
    ):
        diag[key] /= frames
    ...
    diag.update({
        ...
        "temporal_reliability_range": "[0,1]",
