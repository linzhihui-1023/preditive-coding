"""C-V6 main: set-based deep semantic evidence + soft stateful correction.

中文：集合式深历史语义证据 + 软有状态时序修正。

Architecture / 结构：
1. K=4 fixed non-autoregressive history. Only detached frozen C-V3 raw logits
   are stored; C-V6 output never feeds back.
2. Pixel-wise learned history attention keeps t-1..t-4 separate until the
   final semantic evidence is formed. There is no recent-history priority rule.
3. Prediction Error e_t and Dynamics Error epsilon_t remain strictly t-1 based.
4. A small ConvGRU stores correction-decision context only and predicts a soft
   lambda in probability space:
       P_out = (1-lambda) P_current + lambda P_history.
5. Training uses four direct objectives with fixed unit weights:
   - final segmentation NLL,
   - direct history-attention supervision on semantic-rescue pixels,
   - balanced direct gate supervision for Rescue vs Protection,
   - strict RAFT+GT correction-persistence supervision only where consecutive
     frozen C-V3 errors are both semantically rescuable.

No K sweep, no loss-weight sweep, no recursive semantic state is added here.
"""
...
def _pad_history_low(current_probability, candidate_rows, history_length):
    ...
    for index in range(history_length):
        if index < len(candidate_rows):
            ...
            probabilities.append(F.softmax(low_logits.float(), dim=1))
            validity = row["valid_low_path"].float().unsqueeze(1)
            if validity.shape[-2:] != low_size:
                validity = F.interpolate(validity, size=low_size, mode="nearest")
            validities.append(validity.to(current_probability.dtype))
        else:
...
def _low_correction_evidence(
    ...
    current_probability = F.softmax(current_low.float(), dim=1)
...
def _full_resolution_soft_output(c_v3_logits, candidate_rows, corrector_row):
    ...
    for index in range(corrector_row["history_scores"].shape[1]):
        if index < len(candidate_rows):
            ...
            history_probabilities.append(F.softmax(row["logits"].detach().float(), dim=1))
        else:
    ...
    history_available = valid_tensor.any(dim=1, keepdim=True).to(lambda_logit.dtype)
    lambda_probability = torch.sigmoid(lambda_logit) * history_available

    output_probability = (
        (1.0 - lambda_probability) * current_probability
        + lambda_probability * history_probability
    )
    output_probability = output_probability / output_probability.sum(
        dim=1,
        keepdim=True,
    ).clamp_min(1.0e-8)
    return {
...
def _attention_loss(attention, attention_target, rescue):
...
def _strict_persistence_loss(
    ...
    warped_previous_rescue = F.grid_sample(
        previous_rescue.to(
            device=current_correction.device,
            dtype=torch.float32,
        )[None, None],
    ...
    persist_mask = stable_semantic & current_rescue & warped_previous_rescue
...
def _train_sequence(
    ...
    pending_motion, motion_hidden = v5._initialize_motion(observer, residual, frame0, frame1)
    ...
    totals = {
        ...
        "attention_valid_pixels_by_age": [0] * HISTORY_LENGTH,
    }
    ...
    for frame_index in range(2, len(samples)):
        ...
        # First real C-V3 output seeds semantic history only.
        if not raw_history:
        ...
        with torch.no_grad():
            candidate_rows = v5._build_history_candidates(
                raw_history,
                motion_history,
                pending_motion,
                HISTORY_LENGTH,
        ...
        with torch.no_grad():
            ...
            for age in range(HISTORY_LENGTH):
                ...
                if count:
                    ...
                    totals["attention_valid_pixels_by_age"][age] += 1
        ...
        if boundary:
            ...
            if correction_hidden is not None:
                correction_hidden = correction_hidden.detach()
    ...
    for key in (
        ...
        "lambda_mean",
        "lambda_on_rescue",
        "lambda_on_protection",
        "prediction_error_abs",
        "dynamics_error_abs",
    ):
        totals[key] /= supervised_frames
    for age in range(HISTORY_LENGTH):
        denominator = max(totals["attention_valid_pixels_by_age"][age], 1)
        totals["attention_mass_by_age"][age] /= denominator
    return totals
...
def _train_epoch(
    ...
    result["attention_mass_by_age"] = [
        sum(
            row["attention_mass_by_age"][age] * row["supervised_frames"]
            for row in rows
        ) / frame_total
        for age in range(HISTORY_LENGTH)
    ]
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        row = {
            ...
            "delta_vs_c_v3_base": _delta_metrics(
                metrics["c_v6"], metrics["c_v3_base"]
            ),
    ...
    summary = {
        ...
        "source_decision_evidence": {
            ...
            "decision": (
                ...
                "therefore deep history is retained as soft semantic evidence rather than "
        ...
        "losses": {
            ...
            "attention": (
                "uniform mass over every GT-correct valid history on Current-wrong rescue pixels"
            ),
            "gate": (
                "balanced direct Rescue=1 vs Protection=0 supervision"
            ),
            "persistence": (
                "RAFT+GT stable semantic correspondence where both consecutive frames are rescuable"
            ),
