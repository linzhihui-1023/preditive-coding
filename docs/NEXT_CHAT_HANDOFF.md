...
latest implementation commit before this handoff: 53f7d0d
```
...
Future-feature history conditions are:

| Name | Predictor history input |
| --- | --- |
| Copy-current | Predictor bypassed; `Fhat=F_t` |
| Current-only | Zeros |
| Latest | Previous completed instantaneous residual |
| Two-tap | `0.207e_(t-1)+0.793e_(t-2)` |
| Recursive | `0.207e_(t-1)+0.793epsilon_(t-2)` |

Do not call Two-tap “lag-1 error.” Legacy aliases remain accepted only for old
...
/tmp filesystem: about 259 GB free
```

## Seed-0 Feature Matrix Result
...
The full local suite passed 37 tests. The exact `1ae6b27` GitHub Actions unit
test run was also reported successful. CI status for later documentation and
guard commits was not checked during this handoff.
...
- Do not begin noise, blur, or online-adaptation claims yet.

Required gate:

```text
Current-only validation MSE < Copy-current validation MSE
```

The next discussion should choose a clean way to address predictor
generalization before history is tested again. Leading options are:
...
Do not change the error recurrence first. The agreed experiment order remains:
keep the present residual, establish a viable next-feature predictor, then test
state, and modify error design only if state still fails under a sufficient
predictor and credible data split.

## Reproduction Commands

Run the five-condition 1x1 matrix:
