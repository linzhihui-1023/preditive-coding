...
OUTPUT_ROOT="${PREDIFY_STAGE4_MULTIDRIVE_OUTPUT_ROOT:-/tmp/predify-storage/experiments/seed0_stage4_multidrive_${SHORT_REVISION}}"

if [[ "$(git -C "$REPO_ROOT" branch --show-current)" != "predify-selective-adaptation-v2" ]]; then
    echo "Stage-4 multi-drive experiment must run from predify-selective-adaptation-v2." >&2
    exit 2
fi
if [[ "$REPO_ROOT" != "/home/lin/predify2021_selective_adaptation" ]]; then
    echo "Stage-4 multi-drive experiment must use the isolated worktree." >&2
    exit 3
fi
if [[ -n "$(git -C "$REPO_ROOT" status --porcelain)" ]]; then
    echo "Refusing a formal multi-drive run from a dirty worktree." >&2
    exit 4
fi
if [[ -e "$OUTPUT_ROOT" ]]; then
    echo "Refusing to overwrite multi-drive output: $OUTPUT_ROOT" >&2
    exit 5
fi
mkdir -p "$OUTPUT_ROOT"

TRAIN_DRIVES="2011_09_26/2011_09_26_drive_0005_sync,2011_09_26/2011_09_26_drive_0013_sync,2011_09_26/2011_09_26_drive_0014_sync,2011_09_26/2011_09_26_drive_0036_sync"
VAL_DRIVES="2011_09_26/2011_09_26_drive_0011_sync,2011_09_26/2011_09_26_drive_0039_sync"

# Deliberately excludes frozen Test drive variables. Training sees only Train/Val.
TRAIN_ENV=(
    "HOME=/home/lin"
    "LANG=C.UTF-8"
    "TZ=Asia/Shanghai"
    ...
    "PREDIFY_TRAIN_DRIVES=$TRAIN_DRIVES"
    "PREDIFY_VAL_DRIVES=$VAL_DRIVES"
    "PREDIFY_FORMAL_SPLIT=1"
    "PREDIFY_SAME_DRIVE_SPLIT=0"
    "PREDIFY_SAME_DRIVE_THREE_WAY_SPLIT=0"
    ...
    "PREDIFY_FEATURE_HISTORY_MODE=aligned_difference"
    "PREDIFY_TEMPORAL_FUSION_MODE=none"
...
# The evaluator owns the frozen split constants and atomically claims one Test read.
env -i \
    "${TRAIN_ENV[@]}" \
    "PREDIFY_MULTIDRIVE_CHECKPOINT=${PREFIX}_best_student.pt" \
    "PREDIFY_MULTIDRIVE_EVAL_OUTPUT_DIR=$OUTPUT_ROOT/evaluation" \
    "$PYTHON_BIN" -u -m predify2021.mce_scores.evaluate_kitti_stage4_multidrive \
