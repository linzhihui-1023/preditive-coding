...
OUTPUT_ROOT="${PREDIFY_STAGE4_SAME_DRIVE_OUTPUT_ROOT:-/tmp/predify-storage/experiments/seed0_stage4_same_drive_60_20_20_${SHORT_REVISION}}"

if [[ "$(git -C "$REPO_ROOT" branch --show-current)" != "predify-selective-adaptation-v2" ]]; then
    echo "Stage-4 same-drive diagnostic must run from predify-selective-adaptation-v2." >&2
    exit 2
fi
if [[ "$REPO_ROOT" != "/home/lin/predify2021_selective_adaptation" ]]; then
    echo "Stage-4 same-drive diagnostic must use the isolated worktree." >&2
    exit 3
fi
if [[ -n "$(git -C "$REPO_ROOT" status --porcelain)" ]]; then
    echo "Refusing a formal same-drive run from a dirty worktree." >&2
    exit 4
fi
if [[ -e "$OUTPUT_ROOT" ]]; then
    echo "Refusing to overwrite same-drive output: $OUTPUT_ROOT" >&2
    exit 5
fi
mkdir -p "$OUTPUT_ROOT"
...
COMMON_ENV=(
    ...
    "PREDIFY_SAME_DRIVE_THREE_WAY_SPLIT=1"
    "PREDIFY_TRAIN_FRACTION=0.6"
    "PREDIFY_VAL_FRACTION=0.2"
    "PREDIFY_TEST_FRACTION=0.2"
...
PREFIX="$OUTPUT_ROOT/stage4_same_drive_60_20_20_seed0"
printf 'git_revision=%s\noutput_root=%s\ncondition=stage4_aligned_temporal_difference\ndrive=0005\nraw_frame_split=train_first_60,val_middle_20,test_last_20\nshuffle=false\nselection_split=val\ntest_access=once_after_best_val_selection\nfuture_feature_stage=4\ntarget_flow_top_stage=5\nmetric_gate=same-segment-stage4-copy-current\n' \
...
env -i \
    "${COMMON_ENV[@]}" \
    "PREDIFY_SAME_DRIVE_DIAGNOSTIC_CHECKPOINT=${PREFIX}_best_student.pt" \
    "PREDIFY_SAME_DRIVE_DIAGNOSTIC_OUTPUT_DIR=$OUTPUT_ROOT/evaluation" \
    "$PYTHON_BIN" -u -m predify2021.mce_scores.evaluate_kitti_stage4_same_drive_diagnostic \
