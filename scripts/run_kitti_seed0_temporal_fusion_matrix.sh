...
OUTPUT_ROOT="${PREDIFY_TEMPORAL_FUSION_OUTPUT_ROOT:-/tmp/predify-storage/experiments/seed0_temporal_fusion_matrix_${SHORT_REVISION}}"

if [[ "$(git -C "$REPO_ROOT" branch --show-current)" != "predify-selective-adaptation-v2" ]]; then
    echo "Temporal-fusion training must run from predify-selective-adaptation-v2." >&2
    exit 2
fi
if [[ "$REPO_ROOT" != "/home/lin/predify2021_selective_adaptation" ]]; then
    echo "Temporal-fusion training must use the isolated selective-adaptation worktree." >&2
    exit 3
fi
if [[ -n "$(git -C "$REPO_ROOT" status --porcelain)" ]]; then
    echo "Refusing a formal temporal-fusion run from a dirty worktree." >&2
    exit 4
fi
if [[ -e "$OUTPUT_ROOT" ]]; then
    echo "Refusing to overwrite temporal-fusion output: $OUTPUT_ROOT" >&2
    exit 5
fi
mkdir -p "$OUTPUT_ROOT"
...
run_group() {
    ...
    env -i \
        "${COMMON_ENV[@]}" \
        "PREDIFY_EPOCHS=$epochs" \
        "PREDIFY_FEATURE_HISTORY_MODE=$history_mode" \
        "PREDIFY_TEMPORAL_FUSION_MODE=$fusion_mode" \
...
printf 'git_revision=%s\noutput_root=%s\ngroups=copy_current current_only temporal_fusion\ntrain_drive=0005\nval_drive=0011\n' \
    "$GIT_REVISION" "$OUTPUT_ROOT" > "$OUTPUT_ROOT/manifest.txt"

# Copy-current has no parameters, so one exact pass is sufficient.
run_group copy_current copy_current none 1
run_group current_only none none 10
run_group temporal_fusion none two_frame_residual 10

CHECKPOINTS="copy_current=$OUTPUT_ROOT/copy_current_seed0_best_student.pt,current_only=$OUTPUT_ROOT/current_only_seed0_best_student.pt,temporal_fusion=$OUTPUT_ROOT/temporal_fusion_seed0_best_student.pt"
env -i \
    "${COMMON_ENV[@]}" \
    "PREDIFY_TEMPORAL_FUSION_CHECKPOINTS=$CHECKPOINTS" \
    "PREDIFY_TEMPORAL_FUSION_EVAL_OUTPUT_DIR=$OUTPUT_ROOT/evaluation" \
    "$PYTHON_BIN" -u -m predify2021.mce_scores.evaluate_kitti_temporal_fusion_matrix \
...
for label in ("copy_current", "current_only", "temporal_fusion"):
    with (root / f"{label}_seed0.p").open("rb") as handle:
        histories[label] = pickle.load(handle)
with (root / "training_history.json").open("w") as handle:
